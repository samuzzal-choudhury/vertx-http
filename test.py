...
def hello():
    #submitted_file = request.files['manifest']
    request.files['manifest'].save('/tmp/pom.xml')
    #printfile = open('/tmp/pom.xml', 'rb')
    #print (printfile.readlines())
    #printfile.close()
    auth_token = "eyJhbGciOiJSUzI1NiIsImtpZCI6IjBsTDB2WHM5WVJWcVpNb3d5dzh1TkxSX3lyMGlGYW96ZFFrOXJ6cTJPVlUiLCJ0eXAiOiJKV1QifQ..LH_8GuKMcjummZifHoRRW4LD766TnVqlH2-fAu1uxWlvn5a1B8wwrI42g6xomkWqJXxzJlXfxaRLOI5rLB3Efjdy0qefw4WHnNmmGmPyrdBI4N6HbpER7p375nrpnOUpjEOueDKdwlyWUQL2cKWfsCBC1oIn1APUMMA1jpGPe3PgMH6DqK5efk1ItNr1BzDkfMrnnTTFbonW9fRJW3Cvrev9lc0B-lLpbkYJCAmkAbEJIqtJyoiIaVTze-DFtx-5tlsRXpxHI23yauZcEM-VkYr72cNV2zn2_JPHs2_v-NAN-NDMGosdkZD64BLliSUF3enzvMbP4kG7FMUnyy38iA"
    ...
    #res = requests.post(analyticsurl,  headers=header, json=data, params=payload)
    #print (res.text)
    ...
    for index in range(count):
        #dependency_name = str(json_stack_report['result'] [0] ['user_stack_info'] ['dependencies'] [index] ['package'])
        #dependency_version = str(json_stack_report['result'] [0] ['user_stack_info'] ['dependencies'] [index] ['version'])
        #dependency_license = str(json_stack_report['result'] [0] ['user_stack_info'] ['distinct_licenses'] [index])
